...
function HlsEngine({ currentChannel, containerRef, toggleFullscreen, isFullscreen, closePlayer }: any) {
  ...
  useEffect(() => {
    ...
    loadingTimeoutRef.current = setTimeout(() => {
      if (isBuffering || !isPlaying) setShowVpnPopup(true);
    }, 60000);
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, (event, data) => {
        ...
        if (settings.autoPlay) {
          ...
          if (playPromise !== undefined) playPromise.catch(e => console.log("Auto-play prevented", e));
        } else {
          setIsBuffering(false);
        }
      });
      ...
      hls.on(Hls.Events.ERROR, (event, data) => {
        if (data.fatal) setShowVpnPopup(true);
      });
  ...
  const toggleMute = (e?: React.MouseEvent) => {
    ...
    setIsMuted(newMute);
    if (!newMute && volume === 0) { setVolume(1); videoRef.current.volume = 1; }
  };
  ...
  return (
    ...
    >
      <video ref={videoRef} className="w-full h-full object-contain bg-black cursor-pointer" onClick={() => { if (showSettings) setShowSettings(false); else togglePlay(); }} playsInline />
        ...
        {showVpnPopup && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-lg p-4 sm:p-6">
            <div className="bg-zinc-900 border border-white/10 rounded-3xl p-5 sm:p-6 w-full max-w-sm text-center shadow-2xl shadow-red-900/20">
              ...
              <h3 className="text-lg font-bold text-white mb-2">Stream Blocked?</h3>
              <p className="text-zinc-400 text-xs mb-4">This channel took too long to load. Try using a free encrypted VPN to bypass ISP blocks.</p>
              <button onClick={(e) => { e.stopPropagation(); setShowVpnPopup(false); setIsBuffering(false); }} className="w-full py-2.5 bg-white text-black font-bold rounded-xl hover:bg-zinc-200 transition-colors">Dismiss</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
                  ...
                  <AnimatePresence>
                    {showVolumeSlider && (
                      <motion.div initial={{ width: 0, opacity: 0, marginLeft: 0 }} animate={{ width: 80, opacity: 1, marginLeft: 8 }} exit={{ width: 0, opacity: 0, marginLeft: 0 }} className="overflow-hidden flex items-center bg-black/40 backdrop-blur-md rounded-full px-2 h-9 border border-white/5">
                        <input type="range" min="0" max="1" step="0.05" value={isMuted ? 0 : volume} onChange={(e) => { const val = parseFloat(e.target.value); setVolume(val); if(videoRef.current) videoRef.current.volume = val; if(val > 0 && isMuted) setIsMuted(false); if(val === 0) setIsMuted(true); }} className="w-full h-1 bg-white/20 rounded-lg appearance-none cursor-pointer accent-white" />
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
                  ...
                  <AnimatePresence>
                    {showSettings && (
                      <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute bottom-full right-0 mb-3 w-40 bg-black/80 backdrop-blur-2xl border border-white/10 rounded-2xl p-2 shadow-2xl overflow-hidden">
                        ...
                        <button onClick={() => { if(hlsRef.current) hlsRef.current.currentLevel = -1; setCurrentQuality(-1); setShowSettings(false); }} className="w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm font-semibold hover:bg-white/10 transition-colors text-white"><span>Auto</span>{currentQuality === -1 && <Check size={14} className="text-purple-400" />}</button>
                        {qualities.map((level, i) => (
                          <button key={i} onClick={() => { if(hlsRef.current) hlsRef.current.currentLevel = i; setCurrentQuality(i); setShowSettings(false); }} className="w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm font-semibold hover:bg-white/10 transition-colors text-white"><span>{level.height}p</span>{currentQuality === i && <Check size={14} className="text-purple-400" />}</button>
                        ))}
                      </motion.div>
                    )}
                  </AnimatePresence>
...
/*                            REACT-PLAYER ENGINE                             */
/* -------------------------------------------------------------------------- */
function ReactPlayerEngine({ currentChannel, closePlayer, isFullscreen }: any) {
  const { settings } = usePlayerStore();
  return (
    <div className="w-full h-full relative bg-black rounded-2xl overflow-hidden">
      {!isFullscreen && (
        <button onClick={closePlayer} className="absolute top-4 right-4 z-50 p-1.5 sm:p-2 bg-black/60 backdrop-blur-xl rounded-full text-white/70 hover:text-white hover:bg-white/10 border border-white/10 transition-all shadow-lg pointer-events-auto">
          <X size={16} />
        </button>
      )}
      <ReactPlayer 
...
/*                              VIDEO.JS ENGINE                               */
/* -------------------------------------------------------------------------- */
function VideoJsEngine({ currentChannel, closePlayer, isFullscreen }: any) {
  const { settings } = usePlayerStore();
  const videoRef = useRef<HTMLVideoElement>(null);
  ...
  return (
    <div data-vjs-player className="w-full h-full bg-black relative rounded-2xl overflow-hidden">
      {!isFullscreen && (
        <button onClick={closePlayer} className="absolute top-4 right-4 z-50 p-1.5 sm:p-2 bg-black/60 backdrop-blur-xl rounded-full text-white/70 hover:text-white hover:bg-white/10 border border-white/10 transition-all shadow-lg pointer-events-auto">
          <X size={16} />
        </button>
      )}
      <video ref={videoRef} className="video-js vjs-default-skin vjs-big-play-centered w-full h-full" />
    </div>
  );
}
