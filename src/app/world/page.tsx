  ...
  if (isLoading) {
    return (
      <div className="flex-1 h-full flex flex-col items-center justify-center min-h-[50vh]">
        ...
        <p className="mt-8 text-secondary-text font-bold tracking-[0.2em] uppercase text-sm md:text-base">Loading Global Database...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full space-y-6 md:space-y-10 pb-32 md:pb-12">
      ...
      <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-primary-text leading-tight mb-2 sm:mb-4 tracking-tight flex items-center gap-4">
            <Globe className="text-primary hidden sm:block" size={48} />
            {t('world.title')}
          </h1>
        ...
        <div className="relative group w-full lg:w-[400px] xl:w-[500px]">
          ...
          <input
            type="text"
            className="w-full pl-12 pr-6 py-4 md:py-5 text-base md:text-lg bg-surface border border-border-light rounded-[12px] focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all placeholder-muted-text text-primary-text shadow-sm"
      ...
      <div className="flex overflow-x-auto gap-3 md:gap-4 pb-4 pt-2 scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
        <button
          ...
          className={`px-5 py-2.5 rounded-[8px] font-bold whitespace-nowrap transition-all ${
            worldSelectedGroup === 'All' 
              ? 'bg-primary text-white shadow-md scale-105' 
              : 'bg-surface text-secondary-text hover:bg-background border border-border-light'
          }`}
        >
          ...
          {displayedChannels.map((channel, idx) => {
            ...
            return (
              <motion.div
                ...
                key={channel.id}
                className={`relative group rounded-[12px] overflow-hidden bg-surface cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-md ${
                  isPlaying ? 'ring-2 ring-primary shadow-lg -translate-y-1' : 'border border-border-light hover:border-primary/50'
                }`}
                onClick={() => setCurrentChannel(channel)}
              >
                <div className="aspect-video bg-background flex items-center justify-center p-3 md:p-4 relative border-b border-border-light">
                  {channel.logo ? (
                    <img 
                      src={channel.logo} 
                      alt={channel.name}
                    ...
                    <Globe className="w-12 h-12 md:w-16 md:h-16 text-muted-text" />
                  )}
                  
                  {/* Hover Overlay */}
                  <div className="absolute inset-0 bg-surface/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <div className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-primary flex items-center justify-center shadow-md transform scale-50 group-hover:scale-100 transition-transform duration-300 delay-75">
                  ...
                  <button 
                    ...
                    className={`absolute top-4 right-4 p-3 bg-surface/80 backdrop-blur-md border border-border-light rounded-full transition-all duration-300 hover:bg-surface hover:scale-110 shadow-sm ${isFav ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                  >
                    <Heart size={20} className={isFav ? "fill-accent text-accent" : "text-muted-text hover:text-primary"} />
                  </button>
                </div>
                ...
                <div className="p-4 md:p-6 bg-surface">
                  ...
                  <p className="text-xs md:text-sm text-muted-text mt-1 uppercase tracking-widest font-semibold truncate">
                    {channel.group}
                  </p>
                </div>
              </motion.div>
